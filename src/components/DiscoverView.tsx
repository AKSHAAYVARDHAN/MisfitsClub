...
export const DiscoverView: React.FC<DiscoverViewProps> = ({
  ...
  const getConnectionForProfile = (person: PublicProfile) => {
    ...
    const deterministicId = `conn_${[userId, personId].sort().join('_')}`;
    return connections.find(
      (c) =>
        c.id === deterministicId ||
        c.profileId === personId ||
        c.profileId === person.id ||
        c.profileId === person.uid ||
        (c.requesterId === userId && (c.targetId === personId || c.targetId === person.uid || c.profileId === personId)) ||
        (c.targetId === userId && (c.requesterId === personId || c.requesterId === person.uid || c.profileId === personId)) ||
        (c.participants && c.participants.includes(personId) && c.participants.includes(userId))
    );
  };
                  ...
                  {conn?.status === 'connected' ? (
                    ...
                    conn.requesterId === currentUserId ? (
                      <button
                        ...
                        disabled
                        className="inline-flex items-center gap-1.5 border border-blue-500/30 bg-blue-500/10 text-blue-400 px-3 py-2 text-[11px] font-mono-code font-bold uppercase tracking-wider cursor-default opacity-90"
                      >
